...
def experiment(model_name, th, cfg):
    ...
    for s in n_sub:
        ...
        for t in test_trial:
            ...
            acc.append(accuracy)
            # 逐次学習  
            model_sequential_train(model_name, model, x_test, y_test, y_prd, th, s, cfg)
            
        # if 'opt_nu' not in model_name:
            save_accuracy(acc, s, th, model_name, cfg)


def model_sequential_train(model_name, model, x_test, y_test, y_prd, th, s, cfg):
...
def save_accuracy(acc, s, th, model_name, cfg):
    path = f'{cfg.path.save}/acc_by_theta/{model_name}/theta_{th}/'
