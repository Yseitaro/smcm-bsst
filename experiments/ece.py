...
def save_accuracy(ece, acc, conf, cfg, model_name):
    path = f'{cfg.path.save}/ece_by_trial/{model_name}_fix-window/'
    os.makedirs(path, exist_ok=True)
    np.savetxt(path + f'ece.csv', ece, delimiter=',')
    np.savetxt(path + f'ece_accuracy.csv', acc, delimiter=',')
    np.savetxt(path + f'ece_confidence.csv', conf, delimiter=',')
...
if __name__ == '__main__':
    ...
    name_datasets = ['kanoga_long','furui_lab']
    # model_names = ['BCMT','BCMG','LDA','sklearnQDA']
    model_names =   ['MLP','CNN']
    ...
    for name_dataset in name_datasets:
        for model_name in model_names:
            ...
            prbs, label = experiment(model_name, cfg)
            ece, acc, conf = expected_calibration_error(prbs, label)
            save_accuracy(ece, acc, conf, cfg, model_name)
